...
class RoomInline(admin.TabularInline):
    ...
    def get_queryset(self, request):
        ...
        We use `self.parent_model` to access the parent `Hostel` object.
        """
        queryset = super().get_queryset(request)

        # Make sure we're dealing with a valid parent model (Hostel)
        if self.parent_model:
            # Filter rooms based on the current hostel instance
            return queryset.filter(hostel=self.parent_model)
        return queryset
