...
from .models import Hostel, Room, BookingRequest, Notification, Customer, Booking, Transaction, ManagerProfile, UserSearchHistory
...
def analytics_view(request):
    ...
    # Bookings per hostel
    bookings_per_hostel = Booking.objects.values('hostel__name').annotate(total=Count('id'))
    ...
    # Revenue generated
    total_revenue = Transaction.objects.aggregate(total=Sum('amount'))['total']
    revenue_per_hostel = Transaction.objects.values('booking__hostel__name').annotate(total=Sum('amount'))

    # Booking requests by status
    requests_by_status = BookingRequest.objects.values('status').annotate(total=Count('id'))

    # Most searched hostels
    most_searched_hostels = UserSearchHistory.objects.values('hostel__name').annotate(search_count=Count('id')).order_by('-search_count')[:5]
