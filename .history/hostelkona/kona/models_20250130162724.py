...
# Generate an encryption key and store it securely (for example, in Django settings)
key = base64.urlsafe_b64encode(os.urandom(32))  # Generate a 32-byte key
...
class Message(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.encrypted_content:
            self.encrypted_content = self.encrypt_message(self.encrypted_content)
